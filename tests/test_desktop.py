...
from dcs_linux.desktop import Desktop, ShortcutStatus, create_shortcut, detect_desktop
...
def test_a_shortcut_launches_the_stable_install_and_is_executable() -> None:
    ...
    result = create_shortcut(system, writer, Desktop.KDE, install)
...
def test_creating_the_same_shortcut_twice_does_not_duplicate_it() -> None:
    ...
    first = create_shortcut(system, writer, Desktop.GNOME, install)
    second = create_shortcut(system, writer, Desktop.GNOME, install)
...
def test_a_shortcut_write_failure_is_an_outcome_not_an_exception() -> None:
    ...
    result = create_shortcut(system, FailingWriter(system), Desktop.KDE, install)
